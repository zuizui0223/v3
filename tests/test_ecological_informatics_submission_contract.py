...
ROUTE = ROOT / "manuscript" / "M4_V3_REC_PUBLICATION_ROUTE_2026-09-11.md"
...
def test_m4_excludes_tnoa_from_claim_ownership() -> None:
    ...
    assert set(manifest["source_repositories"].values()) == {
        "zuizui0223/v3",
        "zuizui0223/rec",
    }
    assert "tnoa" not in {value.lower() for value in manifest["source_repositories"].values()}
    assert "TNOA is excluded" in ROUTE.read_text(encoding="utf-8")
...
def test_findlay_rights_is_explicit_external_blocker() -> None:
    checklist = CHECKLIST.read_text(encoding="utf-8")
